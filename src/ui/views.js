...
export function renderLibraryView({ savedArtists, savedReleaseContexts, savedTrackContexts, savedMomentContexts = [], songNoteContexts = [] }) {
  ...
  const cards = [
    ...
    ...savedMomentContexts.map(({ artist, release, track, moment }) => `<article class="view-card view-card-moment"><img src="${release.cover}" alt=""/><span>SAVED MOMENT · ${Math.floor(moment.position / 60)}:${String(moment.position % 60).padStart(2, '0')}</span><h3>${escapeHtml(track.title)}</h3><p>${escapeHtml(artist.name)} · ${escapeHtml(release.title)}</p><button type="button" data-open-moment="${track.id}" data-position="${moment.position}">Open moment</button></article>`),
  ...
  const empty = `<section class="empty-library"><div class="empty-library-orbit" aria-hidden="true"></div><div class="empty-library-copy"><span>EMPTY LIBRARY</span><h2>Keep the music that stays with you.</h2><p>Save artists, releases, tracks, exact moments, and private song notes. Your progress remains separate, so a save never interrupts the journey.</p><button class="primary-action" type="button" data-return-discover>Start discovering</button></div><ol class="empty-library-features"><li><span>01</span><div><strong>Artists</strong><p>Return to a full catalog chapter.</p></div></li><li><span>02</span><div><strong>Albums & EPs</strong><p>Keep a release without losing its context.</p></div></li><li><span>03</span><div><strong>Moments & notes</strong><p>Remember exactly why a song stayed with you.</p></div></li></ol></section>`;
