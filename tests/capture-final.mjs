...
const documentUrl = process.env.RONDO_URL || pathToFileURL(resolve('preview-test.html')).href;
...
await capture('desktop-discover', { width: 1440, height: 960 });
await capture('mobile-discover', { width: 390, height: 844 });
await capture('desktop-journey', { width: 1440, height: 960 }, async (page) => {
...
await capture('desktop-release', { width: 1440, height: 960 }, async (page) => {
  await page.waitForFunction(() => document.body.dataset.view === 'release');
}, { screen: 'release' });
await capture('mobile-release', { width: 390, height: 844 }, async (page) => {
  await page.waitForFunction(() => document.body.dataset.view === 'release');
}, { screen: 'release' });
...
await capture('desktop-reveal', { width: 1440, height: 960 }, async (page) => {
  await page.waitForFunction(() => document.body.dataset.view === 'release');
  await page.locator('.release-artifact').scrollIntoViewIfNeeded();
}, { screen: 'reveal' });
await capture('desktop-song-room-reveal', { width: 1440, height: 960 }, async (page) => {
  await page.click('#transportPlay');
  await page.click('#openFullPlayer');
...
await capture('mobile-reduced-motion', { width: 390, height: 844 }, async (page) => {
  await page.evaluate(() => document.querySelector('[data-discover-play]')?.click());
  await page.waitForFunction(() => document.body.classList.contains('is-playing'));
  const signalMode = await page.locator('.song-room-wave').getAttribute('data-signal-mode');
  if (signalMode !== 'reduced') throw new Error('Reduced Motion waveform should be static.');
}, { reducedMotion: 'reduce' });

await browser.close();
console.log('Thirteen final QA captures written.');
